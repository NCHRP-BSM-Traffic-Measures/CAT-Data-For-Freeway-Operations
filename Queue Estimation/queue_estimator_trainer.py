...
class QueueEstimatorTrainer(object):
    ...
    def __init__(self, control_file, parameters_file, train_share = 0.75):
        ...
        Keyword arguments:
        control_file -- json file specifying where the ML training data is 
        parameters_file -- json file specifying the different parameters for XGBoost that will define the grid search
        train_share -- percent of data that will be used for training and not final validation/scoring (defaul 0.75)
        """
    ...
    def modify_train_share(self, new_train_share):
        '''
        Modifies train_share if user wants to change after starting instance

        Keyword arguments:
        new_train_share -- percent of data that will be used for training and not final validation/scoring 
        '''
    ...
    def read_Xy_data(self, df_Xy_filepath):
        ...
        Keyword arguments:
        df_Xy_filepath -- filepath with X and y data for ML training and validation
        '''
    ...
    def split_train_test_X_y(self):
        ''' Preparing X and y components by splitting into train and test splits based on train_share 
        '''
    ...
    def temporal_validation(self, df_X, arr_y, model=xgb.XGBRegressor(), folds = 4, measure = 'mse', non_categorical=False):
        ...
        Keyword arguments:
        df_X -- Dataframe with X data
        arry_y -- Array of y values
        model -- ML model type. Should be regressor
        folds -- Number of folds to be used in temporal validation (see citation for implementation)
        measure -- Can be 'mse' for mean squared error or 'accuracy'. MSE preffered as this is a regression task. (default 'mse')
        non_categorical -- Whether or not to calculate accuracies, i.e. prediction task i.e. categorical labels. Should be false for MSE.
        '''
    ...
    def xgboost_temporal_grid_search(self, folds = 4, measure = 'mse', non_categorical=False, stop_condition=0):
        ...
        measure -- Can be 'mse' for mean squared error or 'accuracy'. MSE preffered as this is a regression task. (default 'mse')
        non_categorical -- Whether or not to calculate accuracies, i.e. prediction task i.e. categorical labels. Should be false for MSE.
        stop_condition -- Because grid search can be computationally intensive, option to cut execution short after certain number of parameter combinations tested(default 0)
        '''
        best_acc = 0
        best_mse = 1e100
    ...
    def validate_model(self):
        ''' 
        Validates best model using test data, returns mse and accuracies
        '''
    ...
    def train_best_parameters_on_full_set(self):
        '''
        After a grid search has been run, trains the model on full set of data for export of non-biased model
        '''
        try:
    ...
    def save_best_model(self, filepath):
        ...
        Keyword arguments:
        filepath -- file location to dump best model
        '''
        try:
