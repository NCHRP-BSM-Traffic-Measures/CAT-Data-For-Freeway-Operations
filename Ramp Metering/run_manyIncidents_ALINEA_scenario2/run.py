...
with open("meterConfig.json") as f:
    ...
    for onramp, config in configs.items():
        ...
        if onramp not in accidents.keys():
            controllers[onramp] = fc.ALINEA(env, meters[onramp], 
            ...
            controllers[onramp] = fc.FeedforwardFeedbackALINEA(env, meters[onramp], 
                                                               meteringUpdateFreq=30, 
                                                               gain=70,
                                                               mainlineSetpoint=15,
                                                               distGain=20,
...
for _ in range(totalTime):
    ...
    if env.step % meteringUpdateFreq == 0:
        ...
        for onramp, controller in controllers.items():
            if onramp not in accidents.keys():
                controller.updateMeteringPlan(nearbyDownstreamMainlineOcc=onrampMeteringInductionLoops[onramp].edgeOcc,
                ...
                if (env.step > accidents[onramp]["startTime"]) and (env.step <= accidents[onramp]["endTime"]):
                    ...
                    # Update metering rate considering downstream bottleneck.
                    controller.updateMeteringPlan(nearbyDownstreamOcc=onrampMeteringInductionLoops[onramp].edgeOcc,
                                                  rampQueueIndicator=onrampEntryInductionLoops[onramp].queueIndicator,
